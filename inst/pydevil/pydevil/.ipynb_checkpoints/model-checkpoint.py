...
def model(input_matrix, 
  ...
  ### This is one possible implementation as a hierarchical model ###
    ...
    with pyro.plate("data", n_cells, dim = -2):
        ...
        if torch.sum(torch.isnan(theta / torch.exp(eta))):
            
            print('rate shape')
            print((theta / torch.exp(eta)).shape)

            print("RATE >= 0")
            print(torch.sum((theta / torch.exp(eta)) > 0))
            print(torch.sum(torch.isnan(theta / torch.exp(eta))))

            nan_mask = torch.isnan(theta / torch.exp(eta))

            # Get the indices of NaN elements
            invalid_indices = torch.nonzero(nan_mask, as_tuple=True)

            print("Indices of NaN elements for rate:")
            print(invalid_indices)

            invalid_matrix_elements = torch.exp(eta)[invalid_indices]
            invalid_vector_elements = theta[invalid_indices[1]]

            print('beta vector')
            print(beta.T)

            print('matrix elements')
            print(invalid_matrix_elements)

            print('matrix elements not exponentiated')
            print(eta[invalid_indices])

            print('vector elements')
            print(invalid_vector_elements)
        ...
        # pyro.sample("obs", dist.NegativeBinomial(logits = eta - torch.log(theta) , total_count= torch.clamp(theta, 1e-9,1e9)), obs = input_matrix)
