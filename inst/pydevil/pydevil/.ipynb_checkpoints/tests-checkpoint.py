...
import pandas as pd
from pydevil.utils import get_variance_factor

def test_posterior_null(inference_res,contrast, alpha = 0.05):
    full_cov = inference_res['hyperparams']['full_cov']
    batch_factor = inference_res['hyperparams']['batch_factor']
    variance_factor = get_variance_factor(batch_factor)
    ...
    if full_cov:
        ...
        total_variance = (variance_term + covariance_term * variance_factor)
    else:
        mu_test = (inference_res["params"]["beta"] * contrast.reshape([-1,1])).sum(axis = 0)
    
        variance_term = (inference_res["params"]["variance"]**2 * abs(contrast)).sum(axis = 1)
        total_variance = variance_term * variance_factor
