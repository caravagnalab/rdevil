...
class myOneCycleLR(Optimizer):
    ...
    def __init__(
        ...
        self.current_step = 0
        self.delta_step = (lr - lr * lrd) / (steps / 2)
    ...
    def step(self, closure: Optional[Callable] = None) -> Optional[Any]:
        ...
        for group in self.param_groups:
            if self.current_step == 0:
                group["lr"] = group["lr"] / group['lrd']
                
            if self.current_step <= (group['steps'] / 2):
                group["lr"] += self.delta_step
            else:
                group["lr"] -= self.delta_step
