...
def compute_hessians(input_matrix, model_matrix, coeff, overdispersion, size_factors, full_cov=True):
    ...
    t = trange(n_genes)
    for gene_idx in t:
        solved_hessian = compute_hessian(obs=input_matrix[:,gene_idx], model_matrix=model_matrix, coeff=coeff[:,gene_idx], overdispersion=overdispersion[gene_idx], size_factors=size_factors)
